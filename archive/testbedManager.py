...
while True:
	...
	for vmName in vms:		
		if "VRDEActiveConnection" in vms[vmName]:
			if vms[vmName]["VRDEActiveConnection"] == "\"on\"" and vmName in availableState and vmName not in notAvailableState:
				nasList.append(vmName)
			elif vms[vmName]["VRDEActiveConnection"] == "\"off\"" and vmName in notAvailableState and vmName not in restoreState:
				resList.append(vmName)
	...
	for vmName in vms:
		if "vrde" in vms[vmName] and vms[vmName]["vrde"] == "\"on\"":
			print "Making available!!!!!!!",vms[vmName]
...
#what if one is restoring while someone connects/disconnects?

#need: one process to continuously scan/put in buffers

#other will keep watching buffers and process
